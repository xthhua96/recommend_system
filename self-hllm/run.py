...
from data.utils import bulid_dataloader
...
def convert_str(s):
    try:
        ...
        print(
            f"Unable to convert the string '{s}' to None / Bool / Float / Int, retaining the original string."
        )
        return s
...
def run_loop(local_rank, config_file=None, saved=True, extra_args=[]):
    ...
    if "text_path" in config:
        config["text_path"] = os.path.join(
            config["text_path"], config["dataset"] + ".csv"
        )
    ...
    if config["val_only"]:
        ...
        logger.info(
            f"Trianing Ended"
            + set_color("best valid ", "yellow")
            + f": {best_valid_result}"
        )

        # model evaluation
        test_result = trainer.evaluate(
            test_loader, load_best_model=saved, show_progress=config["show_progress"]
        )
