...
class Data:
    ...
    def _from_scratch(self):
        self.logger.info(
            set_color(
                f"Loading {self.__class__} from scratch with {self.data_split = }.",
                "green",
            )
        )
    ...
    def _load_inter_feat(self, token, dataset_path, item_data=None):
        ...
        df = pd.read_csv(
            inter_feat_path,
            delimiter=",",
            dtype={"item_id": str, "user_id": str, "timestamp": int},
            header=0,
    ...
    def _data_processing(self):
        ...
        self.logger.info(
            f"{self.inter_feat['item_id'].isna().any() = } {self.inter_feat['user_id'].isna().any() = }"
        )
    ...
    def _build_seq(self, train_feat):
        ...
        for i, uid in enumerate(user_list):
            ...
            if save:
                if (
                    self.data_split is None or self.data_split == True
                ) and i - seq_start > max_item_list_len:
                    ...
                    item_list_index.append(
                        slice(seq_start, i)
                    )  # maybe too long but will be truncated in dataloader
    ...
    def get_norm_adj_mat(self):
        ...
        norm_deg = 1.0 / torch.sqrt(torch.where(deg == 0, torch.ones([1]), deg))
